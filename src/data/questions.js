...
Event Loop позволяет JavaScript выполнять асинхронные операции, обрабатывая их по мере завершения, не блокируя при этом главный поток выполнения. Это делает возможным создание отзывчивых и асинхронных веб-приложений.
  ...
  },
  {
    ...
    answer: 'JSX - это синтаксический сахар для React, который позволяет использовать HTML-подобный синтаксис для описания структуры компонентов React, делая код более читабельным и выразительным.'
  },
  {
    id: 3,
    question: 'Что такое Redux?',
    answer: 'Redux - это библиотека для управления состоянием приложения, особенно полезная для приложений с большим и сложным состоянием, обеспечивающая предсказуемый поток данных.'
  },
  {
    id: 4,
    question: 'Что такое компонент в React?',
    answer: 'Компонент в React - это независимый и переиспользуемый блок кода, который отвечает за отрисовку части пользовательского интерфейса.'
  },
  {
    id: 5,
    question: 'Что такое props в React?',
    answer: 'Props (свойства) в React - это конфигурация компонентов, передаваемая от родительского компонента к дочернему, используемая для настройки поведения и вида компонента.'
  },
  {
    id: 6,
    question: 'Что такое state в React?',
    answer: 'State (состояние) в React - это объект, который содержит данные, влияющие на отрисовку компонента, и может изменяться со временем в ответ на действия пользователя или системные события.'
  },
  {
    id: 7,
    question: 'Что такое Hook в React?',
    answer: 'Hook - это специальная функция в React, которая позволяет использовать состояние и другие возможности React без написания классов. Например, useState и useEffect.'
  },
  {
    id: 8,
    question: 'Чем отличается классовый компонент от функционального в React?',
    answer: 'Классовые компоненты могут содержать состояние и методы жизненного цикла, в то время как функциональные компоненты, до появления Hook, были более простыми и служили для отрисовки UI.'
  },
  {
    id: 9,
    question: 'Что такое Virtual DOM?',
    answer: 'Virtual DOM - это концепция, используемая в React и других библиотеках, представляющая легковесную копию реального DOM в памяти, что позволяет оптимизировать обновление интерфейса.'
  },
  {
    id: 10,
    question: 'Что такое контекст (Context) в React?',
    answer: 'Context в React предоставляет способ передачи данных через дерево компонентов без необходимости передавать props на каждом уровне вручную, упрощая доступ к данным в глубоко вложенных компонентах.'
  }
];